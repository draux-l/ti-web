...
type Group = { id: string; name: string; students: string[] }
...
export function InstructorGroups() {
  ...
  const createGroup = () => {
    ...
    if (!value) return
    const id = `g-${Date.now()}`
    setGroups([{ id, name: value, students: [] }, ...prev])
  ...
  const addStudent = () => {
    ...
    setGroups(prev => prev.map(group => group.id === selectedGroup.id ? { ...group, students: [...group.students, value] } : group))
    setStudentName("")
  }

  const removeStudent = (groupId: string, name: string) => {
    setGroups(prev => prev.map(group => group.id === groupId ? { ...group, students: group.students.filter(s => s !== name) } : group))
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-3xl bg-white shadow-sm">
        <CardHeader><CardTitle className="text-lg">Asignar Estudiante al Grupo</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <select value={selectedGroupId} onChange={(e) => setSelectedGroupId(e.target.value)} className="h-10 w-full rounded-full border border-gray-200 bg-slate-50 px-4 text-sm outline-none focus:border-[#00AEEF]">
        ...
        {groups.map(group => (
          <Card key={group.id} className="rounded-3xl bg-white shadow-sm">
            <CardHeader className="pb-1">
              <CardTitle className="flex items-center gap-2 text-lg text-[#1A1A2E]"><Users className="size-5 text-[#00AEEF]" />{group.name}</CardTitle>
              <p className="text-xs text-gray-500">{group.students.length} estudiantes</p>
            </CardHeader>
            <CardContent className="space-y-3">
              {group.students.length === 0 && <p className="rounded-2xl bg-slate-50 p-3 text-sm text-gray-500">Sin estudiantes asignados.</p>}
