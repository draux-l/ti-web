...
type Experience = { id: string; title: string; groupId: string; dueDate: string; attempts: number; enabled: boolean }
...
const initialAssignments: Experience[] = [
  { id: "a-1", title: "Análisis de Fallas en Aceros", groupId: "g-1", dueDate: "2026-04-24", attempts: 3, enabled: true },
  { id: "a-2", title: "Sistemas Hidráulicos Avanzados", groupId: "g-2", dueDate: "2026-04-20", attempts: 2, enabled: false },
]
...
export function InstructorExperiences() {
  ...
  const sortedAssignments = useMemo(() => [...assignments].sort((a, b) => a.dueDate.localeCompare(b.dueDate)), [assignments])
  ...
  const assignExperience = () => {
    ...
    setAssignments([...prev, { id: `a-${Date.now()}`, title: selectedExperience.name, groupId, dueDate, attempts: 3, enabled: true }])
  }

  const updateAttempts = (id: string, delta: number) => {
    setAssignments(prev => prev.map(item => item.id === id ? { ...item, attempts: Math.max(1, item.attempts + delta) } : item))
  }

  const updateDueDate = (id: string, value: string) => {
    setAssignments(prev => prev.map(item => item.id === id ? { ...item, dueDate: value } : item))
  }

  const toggleEnabled = (id: string) => {
    setAssignments(prev => prev.map(item => item.id === id ? { ...item, enabled: !item.enabled } : item))
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-3xl bg-white shadow-sm">
        <CardHeader><CardTitle className="text-lg">Asignar Experiencia al Grupo</CardTitle></CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-4">
          <select value={experienceId} onChange={(e) => setExperienceId(e.target.value)} className="h-10 rounded-full border border-gray-200 bg-slate-50 px-4 text-sm outline-none focus:border-[#00AEEF] md:col-span-2">
            {catalog.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <select value={groupId} onChange={(e) => setGroupId(e.target.value)} className="h-10 rounded-full border border-gray-200 bg-slate-50 px-4 text-sm outline-none focus:border-[#00AEEF]">
            ...
            <Card key={item.id} className="rounded-3xl bg-white shadow-sm">
              <CardContent className="space-y-4 py-6">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div><p className="text-base font-semibold text-[#1A1A2E]">{item.title}</p><p className="text-sm text-gray-500">{group?.name}</p></div>
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${item.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                    {item.enabled ? "Habilitada" : "Vencida/Bloqueada"}
                  </span>
                </div>
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                    <p className="mb-2 text-xs font-semibold text-gray-500 uppercase">Reprogramar fecha</p>
                    <div className="flex items-center gap-2"><CalendarDays className="size-4 text-[#00AEEF]" /><Input type="date" value={item.dueDate} onChange={(e) => updateDueDate(item.id, e.target.value)} className="h-9 rounded-full bg-white" /></div>
                  </div>
                  <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
