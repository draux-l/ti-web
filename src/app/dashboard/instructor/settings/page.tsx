    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Ajusta tus preferencias de cuenta y seguridad del panel instructor.
        </p>
      </div>
        ...
        <Card className="rounded-3xl bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <User className="size-5 text-[#00AEEF]" />
              Perfil de Usuario
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Nombres:</p>
                <Input
                  value={name}
                  onChange={(event) => setName(event.target.value)}
                  className="h-10 flex-1 rounded-full bg-slate-50"
                />
              </div>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Apellidos:</p>
                <Input
                  value={lastName}
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Cargo:</p>
                <Input
                  value={position}
                  onChange={(event) => setPosition(event.target.value)}
                  disabled
                  className="h-10 flex-1 rounded-full bg-slate-50 text-gray-500"
                />
              </div>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Teléfono:</p>
                <div className="relative flex-1">
                  <Phone className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-gray-400" />
                  <Input
                    value={phone}
                    onChange={(event) => setPhone(event.target.value)}
                    className="h-10 rounded-full bg-slate-50 pl-10"
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Especialidad:</p>
                <Input
                  value={specialty}
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <p className="text-sm font-medium text-[#1A1A2E] sm:w-40">Biografía:</p>
                <Input
                  value={bio}
            ...
            <Button className="rounded-full bg-[#00AEEF] text-white hover:bg-[#0098d1]">
      ...
      {tab === "notificaciones" && (
        <Card className="rounded-3xl bg-white shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Bell className="size-5 text-[#FFB800]" />
          ...
          <CardContent className="space-y-3">
            <NotificationItem
              ...
              value={alerts.completions}
              onChange={(value) =>
                setAlerts((prev) => ({ ...prev, completions: value }))
              }
            />
            <NotificationItem
              label="Solicitudes de reintento"
              value={alerts.retries}
              onChange={(value) => setAlerts((prev) => ({ ...prev, retries: value }))}
            />
            <NotificationItem
              label="Alertas de grupos inactivos"
              value={alerts.inactiveGroups}
              onChange={(value) =>
                setAlerts((prev) => ({ ...prev, inactiveGroups: value }))
              }
            />
          </CardContent>
        </Card>
